...
async fn test_string_contains_literal(ctx: &mut ProjectTestContext) {
    ...
    let result = ctx
        ...
        .query(filter(field("_id").contains("ob")).limit(100), None, None)
        .await
...
async fn test_string_contains_literal_empty(ctx: &mut ProjectTestContext) {
    ...
    let result = ctx
        .client
        .collection(&collection.name)
        .query(filter(field("_id").contains("")).limit(100), None, None)
        .await
        .expect("could not query");
...
async fn test_string_contains_field_self(ctx: &mut ProjectTestContext) {
    ...
    let result = ctx
        ...
        .collection(&collection.name)
        .query(
            filter(not(field("title").contains(field("title")))).limit(100),
            None,
            None,
        )
        .await

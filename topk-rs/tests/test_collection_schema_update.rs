...
async fn test_add_index_on_written_field(ctx: &mut ProjectTestContext) {
    ctx.client = Client::new(ctx.client.config().clone().with_retry_config(
    ...
    let ids = ids_matching(ctx, &name, "love", "summary", None, Some(updated.ssn))
        .await
        .expect("SDK retries until the index is ready");
...
async fn test_add_vector_index_on_stored_vectors(ctx: &mut ProjectTestContext) {
    ...
    ctx.client
        ...
        .await
        .unwrap();
    nearest(ctx, &name, None)
        .await
        .expect_err("no vector index yet");
    ...
    assert_eq!(
        wait_for_index(async || nearest(ctx, &name, Some(updated.ssn)).await).await,
        vec!["a", "c"]
    );
}
...
async fn test_writes_during_a_vector_index_add(ctx: &mut ProjectTestContext) {
    ...
    assert_eq!(
        wait_for_index(async || nearest(ctx, &name, None).await)
            .await
            .len(),
        2
    );
...
async fn test_add_index_on_partially_populated_field(ctx: &mut ProjectTestContext) {
    ...
    ctx.client
        ...
        .expect("update failed");
    assert_eq!(
        wait_for_index(async || ids_matching(ctx, &name, "love", "summary", None, None).await)
            .await,
        vec!["with"]
    );
}
...
async fn test_add_vector_index_over_flushed_vectors(ctx: &mut ProjectTestContext) {
    ...
    assert_eq!(
        wait_for_index(async || nearest(ctx, &name, None).await).await,
        vec!["a", "c"]
    );
}

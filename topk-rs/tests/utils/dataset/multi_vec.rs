...
pub fn cast(dt: MatrixValueType, matrix: Matrix) -> Matrix {
    match matrix.values {
        Some(matrix::Values::F32(v)) => match dt {
            MatrixValueType::F32 => Matrix::new(matrix.num_cols, v.values),
            MatrixValueType::F16 => Matrix::new(
                matrix.num_cols,
                v.values
                    .into_iter()
            ...
            MatrixValueType::F8 => Matrix::new(
                matrix.num_cols,
                v.values
                    .into_iter()
                    .map(|v| float8::F8E4M3::from_f32(v))
                    .collect::<Vec<_>>(),
            ),
            MatrixValueType::U8 => Matrix::new(
                matrix.num_cols,
                v.values
                    .into_iter()
                    .map(|v| (v.abs() * 64.0).round() as u8)
                    .collect::<Vec<_>>(),
            ),
            MatrixValueType::I8 => Matrix::new(
                matrix.num_cols,
                v.values
                    .into_iter()
                    .map(|v| (v * 64.0).round() as i8)
                    .collect::<Vec<_>>(),
            ),
            MatrixValueType::Unspecified => unreachable!(),
        },
        _ => unreachable!(),
    }
}
...
#[allow(dead_code)]
pub fn docs(dt: MatrixValueType) -> Vec<Document> {
    // !!! IMPORTANT !!!
    ...
    vec![
        ...
        doc!(
            ...
            "smve" => SparseVector::new(vec![1,5,7], vec![17.0f32, 6.0f32, 97.0f32]),
            "token_embeddings" => cast(dt, Matrix::new(7, vec![
        ...
        doc!(
            ...
            "smve" => SparseVector::new(vec![1,3,5], vec![17.0f32, 6.0f32, 97.0f32]),
            "token_embeddings" => cast(dt, Matrix::new(7, vec![
        ...
        doc!(
            ...
            "smve" => SparseVector::new(vec![2,3,4], vec![17.0f32, 6.0f32, 97.0f32]),
            "token_embeddings" => cast(dt, Matrix::new(7, vec![
        ...
        doc!(
            ...
            "smve" => SparseVector::new(vec![0, 1, 2], vec![17.0f32, 6.0f32, 97.0f32]),
            "token_embeddings" => cast(dt, Matrix::new(7, vec![

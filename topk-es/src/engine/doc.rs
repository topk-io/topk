...
pub fn source(schema: &Schema, filter: &SourceFilter, fields: HashMap<String, Value>) -> Source {
    decode(filter, render(schema, flatten(fields)))
}

pub(crate) fn decode(source: &SourceFilter, fields: HashMap<String, Value>) -> Source {
...
pub(crate) fn flatten(fields: HashMap<String, Value>) -> HashMap<String, Value> {
...
pub(crate) fn render(schema: &Schema, fields: HashMap<String, Value>) -> HashMap<String, Value> {
    fields
        .into_iter()
        .map(|(path, value)| {
    ...
    let fields = doc
        ...
        .map(|(name, value)| {
            ...
            let value = match spec.and_then(|spec| spec.data_type.as_ref()?.data_type.as_ref()) {
                ...
                Some(field_type::DataType::Timestamp(_)) => date::parse_date(value)?,
                _ => value,
            };

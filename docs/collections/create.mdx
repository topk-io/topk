...
description: "Collections are the primary data structure in TopK. They store documents and provide the interface for querying them efficiently."
---
...
To create a collection in TopK, call the `create()` method on `client.collections()`.
...
`index()` method is used to create an index on a field.

This method accepts a single parameter specifying the index type:
...
Optionally, you can pass a `model` parameter and `embedding_type` parameter to the `semantic_index()` method:
...
This method is used to create vector index on a vector field. You can add a vector index on `f32_vector`, `u8_vector`, or `binary_vector` fields.

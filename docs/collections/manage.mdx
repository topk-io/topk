...
<Info>
  In collection schemas, a Python dict or JavaScript plain object field is treated as a `struct()` field. This also works for nested objects.
...
schema = {
    ...
    # "meta": struct({"name": text(), "source": struct({"url": text()})})
    "meta": {
        "name": text(),
        "source": {
            "url": text(),

---
title: "Collections"
description: "Create, list, get, and delete collections."
icon: "database"
---
...
| [`timestamp()`](/sdk/topk-py/schema#timestamp) | Timestamps, stored as milliseconds since UNIX epoch |
...
```python Python
from topk_sdk.schema import struct, text
...
Enables multi-vector search on [`matrix()`](/sdk/topk-py/schema#matrix) fields using the maxsim metric for late-interaction scoring. Enabled by [`multi_vector_index()`](/sdk/topk-py/schema#multi_vector_index). See [multi-vector search](/guides/multi-vector-search) for more information.
...
See [semantic_index()](../sdk/topk-py/schema#semantic-index) for details.

#### NGram Index

Accelerates substring and regular expression filters such as [`contains`](/collections/query#contains) and [`regexp_match`](/collections/query#regexp_match) on text fields. Enabled by [`ngram_index()`](/sdk/topk-py/schema#ngram_index).

<CodeGroup>

```python Python
from topk_sdk.schema import ngram_index, text
...
```typescript Javascript
import { ngramIndex, text } from "topk-js/schema";

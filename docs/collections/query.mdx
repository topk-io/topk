...
.filter(match("fantasy", field="tags") & should("epic", field="tags"))
```
...
.filter(match("fantasy", { field: "tags" }).and(should("epic", { field: "tags" })))
```

```sql SQL
WHERE match('fantasy', tags) AND should('epic', tags)
```

</CodeGroup>

This returns only documents matching `fantasy`, while boosting documents that also match `epic`.

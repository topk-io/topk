...
<Steps>
  ...
  <Step title="Returns answer with source citations">
    Answer consists of the following properties:
...
Facts in the answer are associated with one or more citations. Citations are **numbered**. Each fact includes a of `ref_ids`, whose values are keys in the `refs` object.

The `refs` maps a citation number to a [Search Result](/core/search#search-result), including the matched passage (text, image or page), source document ID, file name, document type, dataset, and any requested metadata.

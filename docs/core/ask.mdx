...
Citations are **numbered**. Each fact includes a `ref_ids` list, whose values correspond to keys in the `refs`.

The `refs` maps a citation number to a [Search Result](/core/search#search-result), including the matched passage (text, image or page), document ID, file name, document type, dataset, and any requested metadata.

...
Search query returns a list of Search Results.
...
| `content_id` | `string` | A unique identifier for this specific content chunk or image                                                                                                   |
| `metadata`   | `object` | Metadata fields attached to the document at upload time — must be explicitly requested, see [Retrieving metadata](#retrieving-metadata)                        |
...
<Tabs>
  ...
  <Tab title="Python SDK" icon="/icons/python.svg">
    ...
    <CodeGroup>
      ...
      for result in client.search(
          ...
          datasets=["policies"],
          top_k=10,
          select_fields=["title", "author", "year"], # metadata fields
      ):
          print(result)
      ```
  ...
  <Tab title="JavaScript SDK" icon="/icons/js.svg">
    ...
    for await (const message of client.search(
      ...
      10,
      undefined,
      ["title", "author", "year"], // metadata fields
    )) {
      console.log(message);
    }
    ```

...
- **Features**: Added [`group_by`](https://docs.topk.io/collections/query#group_by) with count, min, max, sum, and mean aggregates to enable facet queries.
- **Features**: [`Sort`](https://docs.topk.io/sdk/topk-py/query#sort) now supports string fields and multi-field sorting for more flexible result ordering.
- **Performance**: Distance computation kernel optimizations to speed up dense vector search.
- **Performance**: Improved multi-tenant isolation with better query execution scheduler.

## June 2026

- **Features**: Added [`Partitioned Collections`](https://docs.topk.io/sdk/topk-py/index#collection) (Namespaces) for effortless multi-tenancy with fully isolated, on-demand partitions.
- **Features**: Made [`SQL`](https://docs.topk.io/sdk/topk-sql/overview) your Search Query Language for hybrid and multi-vector search—no proprietary query language required.
- **Features**: Introduced [`semantic_index`](https://docs.topk.io/guides/semantic-search) enabling production-ready multi-vector retrieval with a single schema annotation.
- **Performance**: Optimized SMVE kernel to match GPU performance with just 8 CPU threads.
...
- **List Support**: Added [`contains`](https://docs.topk.io/sdk/topk-py/query#contains) and [`in`](https://docs.topk.io/sdk/topk-py/query#in) operators for list fields.
...
- **Performance**: Optimized workload distribution for large-scale deployments.

  ...
  <Tab title="Python SDK" icon="/icons/python.svg">
    <Steps>
      ...
      <Step title="Upload a file">
        ...
        <CodeGroup>
          ```python Sync
          client.dataset("my-docs").wait_for_handle(resp)
          ```
          ```python Async
          await client.dataset("my-docs").wait_for_handle(resp)
          ```
        </CodeGroup>
      </Step>
      <Step title="Ask a question">
        <CodeGroup>
          ...
          ```
          ```python Async
          async for message in client.ask("What was the total net income of Bank of America in 2024?", ["my-docs"]):
              print(message)
          ```
        </CodeGroup>
      </Step>

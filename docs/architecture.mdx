...
The diagram reads left to right so paths from the same client (especially the CLI) fan out without crossing as much as in a purely top-down layout. Mermaid still chooses exact anchor points automatically.

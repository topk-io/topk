...
Create a [FieldSpec](/sdk/topk-py/schema#FieldSpec) type for `timestamp` values.
...
```python
struct(fields: Mapping[str, SchemaFieldSpec]) -> FieldSpec
```
...
| Parameter | Type |
| --------- | ---- |
| `fields` | Mapping[str, [`SchemaFieldSpec`](#schemafieldspec)] |
...
Create a [FieldIndex](/sdk/topk-py/schema#FieldIndex) type for `vector_index` values.
...
```python
keyword_index(type: Literal['text', 'exact'] = "text") -> FieldIndex
```
...
```python
ngram_index() -> FieldIndex
```

Create a [FieldIndex](/sdk/topk-py/schema#FieldIndex) type for `ngram_index` values.
...
```python
SchemaFieldSpec = FieldSpec | Mapping[str, 'SchemaFieldSpec']
```

**Type**

[`FieldSpec`](#fieldspec) | Mapping[str, 'SchemaFieldSpec']

...
```python
create(self, collection_name: str, schema: Mapping[str, schema.FieldSpecInput]) -> Collection
```
...
| --------- | ---- |
| `collection_name` | str |
| `schema` | Mapping[str, [`schema.FieldSpecInput`](/sdk/topk-py/schema#fieldspecinput)] |
...
create(
   self,
   collection_name: str,
   schema: Mapping[str, schema.FieldSpecInput]
)
```

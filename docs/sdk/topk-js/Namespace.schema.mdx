...
```ts
function keywordIndex(indexType?: KeywordIndexType): FieldIndex;
```
...
| Parameter | Type |
| ------ | ------ |
| `indexType?` | [`KeywordIndexType`](/sdk/topk-js/Namespace.schema#keywordindextype) |
...
```ts
function ngramIndex(): FieldIndex;
```
...
```ts
function struct(fields: Record<string, SchemaFieldSpec>): FieldSpec;
```
...
| Parameter | Type |
| ------ | ------ |
| `fields` | `Record`\<`string`, [`SchemaFieldSpec`](/sdk/topk-js/index#schemafieldspec)\> |
...
| <a id="property-topk"></a> `topK?` | `number` | Top-k projected values to keep |
| <a id="property-width"></a> `width?` | `number` | Width of the sparse projection |
...
```ts
type KeywordIndexType = "text" | "exact";
```

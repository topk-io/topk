...
docs = client.collection("articles").query(
    ...
    .sort(field("content_similarity") * 0.6 + field("text_score") * 0.4, False)
    .limit(10)
)
```
...
- The `sort()` function weights the scores, prioritizing semantic meaning (60%) while still considering keyword matches (40%).
...
docs = client.collection("books").query(
    ...
    .sort(field("text_score") * 0.2 + field("title_similarity") * 0.8, False)
    .limit(10)
...
docs = client.collection("documents").query(
    ...
    .sort(0.8 * field("importance") + 0.2 * field("content_score"), False)
    .limit(10)
)
```
...
2. Then, the `sort()` function gives 80% weight to content score and 20% weight to document importance.

...
True Hybrid Search allows you to combine multiple retrieval techniques — such as multi-vector search, hybrid (vector \+ text) search and sort the results using a custom scoring function - **within a single query**.
...
1. Run two separate queries against respective indexes
2. Each query returns its top results (e.g. first 100 candidates)
...
flowchart TD
  ...
  F --> G@{ shape: stadium, label: "Combined 100 top results" }
```
...
**TopK is different.** We scan just a single index(vector \+ keyword) within a single query, ensuring that our "top 100" results are the **actual** top 100 - not just a probabilistic approximation:

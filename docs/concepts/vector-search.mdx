...
* Handle **billion-scale vector search** efficiently, suitable for applications with large datasets
* Achieve **99% recall** in vector search — retrieving 99% of the true top-K nearest neighbors for every query
* Provide **low latency** (< 0.5s) for all queries

## Implement vector search

To perform vector search in TopK, use the `vector_distance()` method to compute distance between two vectors:
...
**Metadata filtering** is used to enforce hard constraints based on structured fields such as
category, time range, price range, availability, user permissions, language, or geographic region.

In a multi-tenant setting, metadata filtering can be also used to scope the search to a specific tenant by filtering documents based on the tenant ID.
See our [multi-tenancy guide](/concepts/multi-tenancy) for more details.

In TopK, you can combine vector search with metadata filtering by using the `filter()` method:

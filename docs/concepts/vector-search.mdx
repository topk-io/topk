...
- Achieve **\> 95% recall** in vector search — consistently retrieving more than 95% of the true top-K nearest neighbors on practical workloads
...
Then, we can run a vector search by using the `vector_distance()` function to compute the distance between the query vector and the vector stored in the document:
...
1. Compute the cosine distance between the query embedding and the `title_embedding` field using the `vector_distance()` function.
2. Store the computed cosine distance in the `title_similarity` field.
3. Return the top 10 results by the `title_similarity` field in a descending order.

## Combinding vector search with metadata filtering

While vector search captures semantic similarity, it fails in cases where you need to match on specific data.
Therefore, it often needs to be combined with **metadata filtering** to enforce hard constraints based on crieteria such as category, time range, price range, availability, user permissions, language, or geographic region.

In TopK, you can combine vector search with metadata filtering by combining `vector_distance()` function with the `filter()` function when performing a query:

...
Then, we can run a vector search by using the `vector_distance()` function to compute distance between the query vector and vector stored in the document:
...
Therefore, it often needs to be combined with **metadata filtering** to enforce hard constraints based on crieteria such as

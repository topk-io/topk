...
After, we query the collection with a multi-vector search using the `fn.semantic_similarity()` function:
...
2. Then, we blend the two scores, giving 70% weight to the full paper and 30% weight to the paragraph.

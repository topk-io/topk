...
In TopK, you can run **multiple vector searches in a single query** and apply a **different ranking score** to individual vector search results.
...
After, we run a multi-vector search by using the `fn.semantic_similarity()` function for calculating semantic similarity on

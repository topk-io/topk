...
flowchart TD
    ...
    D --> E
    E -.-> G[Rerank]
    G --> H@{ shape: stadium, label: "Results collection" }
...
- `multi_vector_distance(field, matrix)`: Computes MaxSim distance for multi-vector (matrix) fields. Requires a [`multi_vector_index()`](/sdk/topk-py/schema#multi_vector_index) on the field. See [multi-vector search](/concepts/multi-vector-search).
...
See [multi-vector search](/concepts/multi-vector-search) for schema setup and ingestion details.
...
The `rerank()` function is used to rerank the results of a query. It accepts the following parameters:
...
- **`fields`** — Document fields to include when reranking. The reranker concatenates these and scores them against the query.
- **`topk_multiple`** — Multiple of top-k to consider for reranking. With `topk(10)` and `topk_multiple=2`, the reranker receives 20 candidates and returns the top 10.

Read more in our [reranking](/concepts/reranking) guide.
...
# or with options
.rerank(model="cohere/rerank-v3.5", query="catcher", fields=["title", "description"], topk_multiple=2)
```
...
// or with options
.rerank({ model: "cohere/rerank-v3.5", query: "catcher", fields: ["title", "description"], topkMultiple: 2 })
```
...
The `query()` method accepts a `consistency` parameter to control read consistency: `indexed`, `balanced` (default), or `strong`. See the [consistency](/concepts/consistency) concept for details.

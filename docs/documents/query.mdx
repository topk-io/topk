...
Use the `select()` function with a `field()` expression to select fields from a document. In the select stage, you can also rename existing fields
...
Use the `topk()` function to return the top `k` results. The `topk` function accepts the following arguments:

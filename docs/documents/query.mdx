...
description: "Use the `query` function to search for documents."
---

import Support from "/snippets/support.mdx";

TopK provides a SQL-inspired syntax for querying documents. It features built-in **semantic search**, **text search**, **vector search**, metadata **filtering** as well as **reranking** capabilities.
...
<Tip>
  You can stack multiple select, filter, topk and rerank stages in a single query.
</Tip>
...
client.collection("books").query(
  select(
    title=field("title"),
    "published_year" # you can also select the static fields directly
  )
  ...
)
```
...
<ParamField path="token" type="string" required={true}>
  String token to match.
  Can also contain multiple terms separated by a delimiter.
</ParamField>

<ParamField path="field" type="string" required={false}>
  Field to match on.
  If not provided, the function will match on all fields.
</ParamField>

<ParamField path="weight" type="number" required={false}>
  Weight to use for matching.
  If not provided, the function will use the default weight(**1.0**).
</ParamField>

<ParamField path="all" type="boolean" required={false}>
  Use `all` parameter when matching on multiple terms.
</ParamField>
...
The `match()` function can be configured to match on multiple terms using a **delimiter**.
...
.filter(match("catcher|rye", "title", 1, true))
```
...
.filter(match("catcher", weight=2.0))
```

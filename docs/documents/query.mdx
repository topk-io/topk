...
select(
    delta=field("published_year").sub(1990).abs()
)
```
...
select(
    clamped_score=field("bm25_score").min(2.0)
)

# Take the lower of critic score vs user rating
select(
    conservative_score=field("critic_score").min(field("user_rating"))
)
```
...
select(
    boosted_score=field("relevance_score").max(1.5)
)

# Take the higher of critic score vs user rating
select(
    best_score=field("critic_score").max(field("user_rating"))
)
```

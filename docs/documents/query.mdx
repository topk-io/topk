...
// TODO write that it supports all of the expressions, also that the fields can be bools
...
All queries **must** have a "collect" stage. Currently, we support the `topk()` and `count()` collectors.
...
<ParamField path="field" type="LogicalExpression" required="true">
  ...
  // TODO clarify which expression/operators are support
</ParamField>

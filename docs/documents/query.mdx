...
- **Sort and Limit stages** - Return the top `k` results based on the provided logical expression
- **Count stage** - Return the total number of documents matching the query
- **Rerank stage** - Rerank the results

All queries **must** have either the Limit or Count collection stage.
...
flowchart TD
    ...
    C --> D[Limit]
    D -.-> E[Rerank]
    E --> F@{ shape: stadium, label: "Results collection" }
    B --> G[Limit]
    G --> H@{ shape: stadium, label: "Results collection" }
    B --> I[Count]
    I --> J@{ shape: stadium, label: "Results count" }
```
...
All queries **must** have a collection stage. Currently, we only support the `limit()` and `count()` collectors.

### sort and limit

The `limit()` collector is typically used in conjunction with `sort()`.

Use the `sort()` function to specify the sorting order and then use the `limit()` function to specify the number of results to return.
...
The `limit()` function accepts the following parameter:
...
Using `limit()` without `sort()` will return results in an undefined order.

This is typically useful to debug queries during development.

<CodeGroup>

```python Python
select("title", "title_similarity")
.limit(10)
```

```js Javascript
select({ title: field("title"), title_similarity: field("title_similarity") })
.limit(10)
```

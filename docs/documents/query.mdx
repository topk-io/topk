...
To perform a text search on your documents, use the `match` function. The `match` function supports full-text search and can be used to search for keywords in a field.
...
docs = client.collection("books").query(
    ...
    .filter(match("title", token="Great"))
...
```python Python
# Filtering by vector similarity
.filter(field("title_similarity") > 0.8)
```

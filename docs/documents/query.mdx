...
The `select` method can be used to select fields from documents. It can be used to select specific fields, rename fields, or include computed fields called function expressions.
...
Also, the collection **must** have a [**keyword index**](http://docs.topk.io/collections/create#keyword-index) typically on the fields you're matching against.
...
The `match()` function is the backbone of text search and keyword filtering in TopK.

It allows you to search for documents that contain specific keywords or phrases.

Note that in order to perform keyword filtering, you need to have a
[**keyword index**](http://docs.topk.io/collections/create#keyword-index) defined in the collection.

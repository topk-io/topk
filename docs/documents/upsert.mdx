...
<Note>Please note that dicts and lists are currently not supported. The only exception for complex types applies to float vectors, byte vectors, and bytes.</Note>

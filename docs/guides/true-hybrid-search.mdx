...
docs = client.collection("articles").query(
    ...
    .sort(field("content_similarity") * 0.6 + field("text_score") * 0.4, asc=False).limit(10)
)
```
...
docs = client.collection("books").query(
    ...
    .sort(field("text_score") * 0.2 + field("title_similarity") * 0.8, asc=False).limit(10)
)
```
...
docs = client.collection("documents").query(
    ...
    .sort(0.8 * field("importance") + 0.2 * field("content_score"), asc=False).limit(10)
)
```

...
<Tip>
TopK has built-in support for [`numpy.ndarray`](https://numpy.org/doc/2.2/reference/generated/numpy.ndarray.html) when ingesting and querying multi-vector (matrix) embeddings. If you pass a 2-D ndarray directly, the matrix value type is inferred from its `dtype` (e.g. `float32`, `float16`, `uint8`, `int8`).
</Tip>
...
docs = client.collection("passages").query(
    ...
    ).sort(field("maxsim"), asc=False).limit(10)
)
...
const docs = await client.collection("passages").query(
  ...
  }).sort(field("maxsim"), false).limit(10)
);

...
docs = client.collection("books").query(
    ...
    # cosine/dot-product: higher score = closer → sort descending
    .sort(field("title_similarity"), asc=False).limit(10)
    # for euclidean distance: lower score = closer → sort ascending
    # .sort(field("title_similarity"), asc=True).limit(10)
)
...
const docs = await client.collection("books").query(
  ...
  // for euclidean distance: lower score = closer → sort ascending
  // }).sort(field("title_similarity"), true).limit(10)
);

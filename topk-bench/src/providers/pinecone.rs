...
pub const PY_CODE: &str = include_str!("pinecone.py");
...
impl PineconeProvider {
    /// Creates a new PineconeProvider.
    pub async fn new() -> anyhow::Result<Provider> {
        let py = PythonProvider::new(PY_CODE).await?;

        Ok(Provider::Pinecone(PineconeProvider { py }))
    }
}

#[async_trait]
impl ProviderLike for PineconeProvider {

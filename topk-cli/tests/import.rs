...
use common::seed::{self, minio, sqlite, Seed};
use common::*;
...
#[test_context(Ctx)]
#[tokio::test]
async fn nothing_read_creates_nothing(ctx: &mut Ctx) {
    let object = ctx.seed_parquet("books", books()).await;
    let collection = ctx.collection("limit-zero");
    let target = Target {
        limit: Some(0),
        ..object
    };
    let spec = ctx.target_spec(&collection, target);
    ok(&["import", "-f", &spec, "--yes"], &[]);
    assert!(
        matches!(
            ctx.client().collections().get(&collection).await,
            Err(topk_rs::Error::CollectionNotFound)
        ),
        "a run that reads nothing must not create the collection"
    );
}

#[test_context(Ctx)]
#[tokio::test]
async fn failed_read_creates_nothing(ctx: &mut Ctx) {
    let object = ctx.seed_parquet("books", books()).await;
    let collection = ctx.collection("bad-filter");
    let target = Target {
        filter: Some("no_such_column = 1".to_string()),
        ..object
    };
    let spec = ctx.target_spec(&collection, target);
    fails(&["import", "-f", &spec, "--yes"], &[]);
    assert!(
        matches!(
            ctx.client().collections().get(&collection).await,
            Err(topk_rs::Error::CollectionNotFound)
        ),
        "a failed read must not leave a collection behind"
    );
}
...
async fn http_roundtrip(ctx: &mut Ctx) {
    ...
    ok(&["import", "-f", &spec, "--yes"], &[]);
    let got = ctx.get(&collection, &["mockingbird"]).await;
    assert_eq!(got["mockingbird"]["title"], topk_rs::proto::v1::data::Value::string("To Kill a Mockingbird"));
}
...
async fn continue_on_error(ctx: &mut Ctx) {
    ...
    let err = fails(&["import", "-f", &spec, "--yes", "--continue-on-error"], &[]);
    assert!(err.contains("1 document(s) skipped"), "got:\n{err}");
...
async fn reimport_is_idempotent(ctx: &mut Ctx) {
    ...
    for _ in 0..2 {
        let run = outcome(&ok(&["import", "-f", &spec, "--yes", "-o", "json"], &[]), &collection);

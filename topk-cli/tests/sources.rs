...
use common::seed::{self as seed, es, mongo, mysql, parquet, pg, sqlite, xlsx, Seed};
use common::*;
...
async fn es_limit() {
    ...
    let docs = stream_docs_from(client.url().as_deref(), &target).await.unwrap();
    assert_eq!(docs.len(), 2);
}
...
async fn mongo_binary() {
    ...
    .await
    .unwrap();
    let docs = stream_docs_from(Some(mongo::Mongo::URL), &target).await.unwrap();
    assert_eq!(
        docs["b1"],

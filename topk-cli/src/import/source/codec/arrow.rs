...
pub fn value(array: &ArrayRef, row: usize) -> Result<Value, Error> {
    ...
    Ok(match array.data_type() {
        ...
        DataType::Float16 => {
            ...
            finite_f64(f64::from(f))?
        }
        DataType::Float32 => finite_f64(f64::from(get!(array::Float32Array)))?,
        DataType::Float64 => finite_f64(get!(array::Float64Array))?,
...
fn finite_f64(f: f64) -> Result<Value, Error> {
    Ok(Value::f64(super::finite(f)?))
}

fn finite_list(values: impl Iterator<Item = f64>) -> Result<Value, Error> {
    Ok(Value::list(
        values.map(super::finite).collect::<Result<Vec<_>, _>>()?,
    ))
}
...
fn list(elements: &ArrayRef) -> Result<Value, Error> {
    ...
    Ok(match elements.data_type() {
        ...
        DataType::Float16 => {
            return finite_list(at!(array::Float16Array).map(|n| f64::from(f32::from(n))))
        }
        DataType::Float32 => return finite_list(at!(array::Float32Array).map(f64::from)),
        DataType::Float64 => return finite_list(at!(array::Float64Array)),
...
fn matrix(rows: &ArrayRef) -> Result<Value, Error> {
    ...
    for (i, row) in rows.iter().enumerate() {
        let row: Vec<f32> = super::floats(&list(row)?)

...
impl FromStr for Uri {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        // elsewhere `?` is a glob character, so only http gets stripped.
            ...
            other => {
                ...
                return Err(Error::InvalidArgument(format!(
                    "cannot tell the file type of {s:?}: expected a csv, tsv, json, jsonl, \
                     ndjson, parquet, avro or xlsx extension{hint}"
                )));
            }
        };

        Ok(Self::File {
            // A quoted glob never reaches the shell; `~` is meaningless in a
            // bucket key.
            path: match store {

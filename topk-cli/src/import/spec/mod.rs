...
impl Spec {
    ...
    /// Every rule a spec must hold to, whether a user wrote it or `discover`
    /// derived it from a source — the two paths used to disagree on field names,
    /// so a source column like `_class` reached the server instead of the user.

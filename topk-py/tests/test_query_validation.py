...
def test_query_topk_by_non_primitive(ctx: ProjectContext):
    ...
    with pytest.raises(
        ...
        match="Input to SortWithLimit must produce primitive type, not String",
    ):
...
def test_query_topk_by_non_existing(ctx: ProjectContext):
    ...
    with pytest.raises(
        error.InvalidArgumentError,
        match="Input to SortWithLimit must produce primitive type, not Null",
    ):
        ctx.client.collection(collection.name).query(
...
def test_query_topk_limit_zero(ctx: ProjectContext):
    ...
    with pytest.raises(
        error.InvalidArgumentError, match="Invalid argument: TopK k must be > 0"
    ):
        ctx.client.collection(collection.name).query(
...
def test_union_u32_and_binary(ctx: ProjectContext):
    ...
    with pytest.raises(
        error.InvalidArgumentError,
        match="Input to SortWithLimit must produce primitive type",
    ):
        ctx.client.collection(collection.name).query(

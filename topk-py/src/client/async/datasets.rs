...
impl AsyncDatasetsClient {
    ...
    pub fn list(&self, py: Python<'_>) -> PyResult<Py<PyAny>> {
        ...
        future_into_py(py, async move {
            let datasets = client.datasets().list().await.map_err(RustError)?;
            let datasets: Vec<Dataset> = datasets.into_iter().map(|i| i.into()).collect();
            Ok(datasets)
        })
        .map(|result| result.into())
    }
    ...
    pub fn update(
        ...
        future_into_py(py, async move {
            let dataset = client
                .datasets()
                .update(&dataset_name, description)
                .await
                .map_err(RustError)?;
            Ok(Dataset::from(dataset))
        })

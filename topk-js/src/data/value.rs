...
use crate::data::vector::{SparseVectorData, SparseVectorUnion, VectorUnion};
    ...
    unsafe fn from_napi_value(
        ...
        match result {
            ...
            napi::sys::ValueType::napi_object => {
                // Vectors
                if let Ok(array) = Vec::<f64>::from_napi_value(env, value) {
                    return Ok(Value::Vector(Vector::float(
                        array.into_iter().map(|v| v as f32).collect(),
                    )));
                }

import { field, literal, LogicalExpression, LogicalExpressionUnion } from '../lib/query';
...
describe('Expression Tests', () => {
  ...
  it('should throw errors for invalid operations', () => {
    expect(() => literal(1).add('string' as any)).toThrow("Unsupported numeric type: 4");
    expect(() => field('a').and(1 as any)).toThrow("Unsupported boolish type: 3");
    expect(() => field('a').or(1 as any)).toThrow("Unsupported boolish type: 3");
    expect(() => field('a').add(null)).toThrow("Unsupported numeric type: 1");
    expect(() => field('a').add('string' as any)).toThrow("Unsupported numeric type: 4");
    expect(() => field('a').add([1, 2, 3] as any)).toThrow("Unsupported numeric type: 6");
    expect(() => field('a').add({ a: 1 } as any)).toThrow("Unsupported numeric type: 6");
  });
});

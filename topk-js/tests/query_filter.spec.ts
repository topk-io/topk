...
describe('Filter Queries', () => {
  ...
  test('query starts with', async () => {
    ...
    const results = await ctx.client.collection(collection.name).query(
      select({}).filter(field('_id').startsWith('cat')).topK(field('published_year'), 100, false)
    );
  ...
  test('query starts with empty', async () => {
    ...
    const results = await ctx.client.collection(collection.name).query(
      select({}).filter(field('_id').startsWith('')).topK(field('published_year'), 100, false)
    );

    expect(new Set(results.map(doc => doc._id))).toEqual(new Set([
      'gatsby',
      'catcher',
      'moby',
      'mockingbird',
  ...
  test('query starts with non-existent prefix', async () => {
    ...
    const results = await ctx.client.collection(collection.name).query(
      select({}).filter(field('_id').startsWith('foobarbaz')).topK(field('published_year'), 100, false)
    );

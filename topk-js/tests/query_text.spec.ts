...
describe('Text Queries', () => {
  ...
  test('query text filter single term disjunctive', async () => {
    ...
    await ctx.client.collection(collection.name).upsert([
      { _id: 'pride', summary: 'A story about love and class', published_year: 1813 },
      { _id: 'gatsby', summary: 'A tale of love and wealth', published_year: 1925 },
    ...
    expect(new Set(result.map(doc => doc._id))).toEqual(new Set(['pride', 'gatsby']));
  });
  ...
  test('query text filter two terms disjunctive', async () => {
    ...
    await ctx.client.collection(collection.name).upsert([
      { _id: 'pride', title: 'Pride and Prejudice', summary: 'A story about love and class', published_year: 1813 },
      { _id: 'gatsby', title: 'The Great Gatsby', summary: 'A tale of lust and wealth', published_year: 1925 },
      { _id: 'lotr', title: 'The Lord of the Rings', summary: 'A fantasy epic', published_year: 1954 },
    ]);
    ...
    expect(new Set(result.map(doc => doc._id))).toEqual(new Set(['pride', 'lotr']));
  });
  ...
  test('query select bm25 without text queries', async () => {
    ...
    ).rejects.toThrow('invalid argument: Invalid query: Query must have at least one text filter to compute bm25 scores');
  });
});
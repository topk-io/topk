...
async fn drop_index_keeps_the_column() {
    let rows = TableScope::with_scope(async |ctx| {
        ...
        ctx.sql("INSERT INTO {{table}} (_id, title) VALUES ('a', 'Dune')")
            .await?;
        ctx.sql(&format!("DROP INDEX {}_title_idx ON {}", ctx.name, ctx.name))
            .await?;
        ctx.sql(&format!(
            "DROP INDEX IF EXISTS {}_missing_idx ON {}",
            ctx.name, ctx.name
        ))
        .await?;
...
#[case::unknown_index("DROP INDEX {{table}}_nope_idx ON {{table}}", "does not exist")]
#[case::drop_index_without_a_collection("DROP INDEX {{table}}_title_idx", "requires `ON <collection>`")]
